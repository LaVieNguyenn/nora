#!/bin/bash
# Nora - pkgutil receipt helpers.
# Finds package-installed app bundles outside the standard app locations.

set -euo pipefail

if [[ -n "${NORA_PKG_RECEIPTS_LOADED:-}" ]]; then
    return 0
fi
readonly NORA_PKG_RECEIPTS_LOADED=1
...
pkg_receipt_nonstandard_app_paths() {
    ...
    local cache_file="${NORA_PKG_RECEIPT_CACHE_FILE:-$HOME/.cache/nora/pkg_receipt_apps_v1}"
    local cache_ttl="${NORA_PKG_RECEIPT_CACHE_TTL:-3600}"
    local now_epoch=0
    ...
    if [[ "${NORA_PKG_RECEIPT_CACHE_DISABLE:-0}" != "1" && -r "$cache_file" ]]; then
        local cache_mtime=0
    ...
    if declare -f run_with_timeout > /dev/null 2>&1; then
        pkgs_output=$(run_with_timeout "${NORA_PKG_RECEIPT_LIST_TIMEOUT:-3}" pkgutil --pkgs 2> /dev/null || true)
    else
    ...
    local scan_timeout="${NORA_PKG_RECEIPT_SCAN_TIMEOUT:-8}"
    local pkg_id
    ...
    if [[ "${NORA_PKG_RECEIPT_CACHE_DISABLE:-0}" != "1" && -n "$cache_file" ]]; then
        ...
        local cache_tmp
        cache_tmp=$(mktemp "${TMPDIR:-/tmp}/nora.pkg_receipts.XXXXXX" 2> /dev/null || true)
        if [[ -n "$cache_tmp" ]]; then
            if [[ ${#seen_apps[@]} -gt 0 ]]; then
