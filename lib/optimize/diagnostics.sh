...
readonly NORA_OPTIMIZE_DIAG_CPU_THRESHOLD_DEFAULT=25
readonly NORA_OPTIMIZE_DIAG_SAMPLE_DELAY_DEFAULT=1

opt_diag_cpu_threshold() {
    local threshold="${NORA_OPTIMIZE_DIAG_CPU_THRESHOLD:-$NORA_OPTIMIZE_DIAG_CPU_THRESHOLD_DEFAULT}"
    if ! [[ "$threshold" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
        threshold="$NORA_OPTIMIZE_DIAG_CPU_THRESHOLD_DEFAULT"
    fi
    printf '%s\n' "$threshold"
}

opt_diag_sample_delay() {
    local delay="${NORA_OPTIMIZE_DIAG_SAMPLE_DELAY:-$NORA_OPTIMIZE_DIAG_SAMPLE_DELAY_DEFAULT}"
    if ! [[ "$delay" =~ ^[0-9]+([.][0-9]+)?$ ]]; then
        delay="$NORA_OPTIMIZE_DIAG_SAMPLE_DELAY_DEFAULT"
    fi
    printf '%s\n' "$delay"
}
...
opt_diag_get_ps_sample() {
    ...
    case "$index" in
        1) override="${NORA_OPTIMIZE_PS_SAMPLE_1:-}" ;;
        2) override="${NORA_OPTIMIZE_PS_SAMPLE_2:-}" ;;
    esac
...
opt_diag_get_spctl_status() {
    if [[ -n "${NORA_OPTIMIZE_SPCTL_STATUS:-}" ]]; then
        printf '%s\n' "$NORA_OPTIMIZE_SPCTL_STATUS"
        return 0
    fi
...
opt_diag_get_hdiutil_info() {
    if [[ -n "${NORA_OPTIMIZE_HDIUTIL_INFO:-}" ]]; then
...
opt_diag_family_note() {
    case "$1" in
        cloudshell)
            printf '%s\n' "External enterprise agent pressure detected. Nora will not terminate enterprise security processes; restart or policy checks must happen outside Nora."
            ;;
        syspolicyd)
...
opt_diag_offer_detach_candidates() {
    ...
    if ! key=$(read_key); then
        echo -e "\n  ${GRAY}${ICON_WARNING}${NC} Kept mounted, whitelist via ${NC}nr optimize --whitelist${GRAY}${NC}"
        return 0
    fi
...
run_optimize_diagnostics() {
    ...
    if [[ -z "${NORA_OPTIMIZE_PS_SAMPLE_1:-}" || -z "${NORA_OPTIMIZE_PS_SAMPLE_2:-}" ]]; then
        sleep "$delay"
    fi
